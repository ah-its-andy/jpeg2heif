...
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -ldflags '-extldflags "-static"' -o jpeg2heif ./cmd/jpeg2heif
...
COPY --from=builder /build/jpeg2heif /usr/local/bin/jpeg2heif
